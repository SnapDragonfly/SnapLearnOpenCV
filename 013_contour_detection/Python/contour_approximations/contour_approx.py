...
cv2.waitKey(0)
cv2.imwrite('contour_approximation_simple.jpg', image_copy2)
...
# to actually visualize the effect of `CHAIN_APPROX_NONE`, we need a proper image
contours3, hierarchy3 = cv2.findContours(thresh1, cv2.RETR_TREE, 
                                                cv2.CHAIN_APPROX_NONE)
image_copy3 = image1.copy()
cv2.drawContours(image_copy3, contours3, -1, (0, 255, 0), 2, 
                 cv2.LINE_AA)
cv2.imshow('None Approximation contours', image_copy3)
cv2.waitKey(0)
cv2.imwrite('contour_approximation_none.jpg', image_copy3)
image_copy4 = image1.copy()
for i, contour in enumerate(contours3): # loop over one contour area
    for j, contour_point in enumerate(contour): # loop over the points 
        # draw a circle on the current contour coordinate
        cv2.circle(image_copy4, ((contour_point[0][0], contour_point[0][1])), 
                   2, (0, 255, 0), 2, cv2.LINE_AA)
# see the results
cv2.imshow('CHAIN_APPROX_SIMPLE Point only', image_copy4)
cv2.waitKey(0)
cv2.imwrite('contour_point_none.jpg', image_copy4)
cv2.destroyAllWindows()