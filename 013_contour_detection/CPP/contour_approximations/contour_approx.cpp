...
int main() {
    ...
    findContours(thresh1, contours2, hierarchy2, RETR_TREE, CHAIN_APPROX_SIMPLE);
    Mat image_copy2 = image1.clone();
    drawContours(image_copy2, contours2, -1, Scalar(0, 255, 0), 2);
    imshow("None approximation", image_copy2);
    waitKey(0);
    imwrite("contour_approximation_simple.jpg", image_copy2);
    destroyAllWindows();
    ...
    // using a proper image for visualizing CHAIN_APPROX_NONE
    vector<vector<Point>> contours2;
    vector<Vec4i> hierarchy2;
    findContours(thresh1, contours2, hierarchy2, RETR_TREE, CHAIN_APPROX_NONE);
    Mat image_copy3 = image1.clone();
    drawContours(image_copy3, contours2, -1, Scalar(0, 255, 0), 2);
    imshow("None approximation", image_copy3);
    waitKey(0);
    imwrite("contour_approximation_none.jpg", image_copy3);
    destroyAllWindows();
    Mat image_copy4 = image1.clone();
    for(int i=0; i<contours2.size(); i=i+1){
        for (int j=0; j<contours2[i].size(); j=j+1){
            circle(image_copy4, (contours2[i][0], contours2[i][1]), 2, 
            Scalar(0, 255, 0), 2);
        }
    }
    imshow("CHAIN_APPROX_NONE Point only", image_copy4);
    waitKey(0);
    imwrite("contour_point_none.jpg", image_copy4);
    destroyAllWindows();
}
